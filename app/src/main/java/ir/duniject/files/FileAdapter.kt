...
import ir.duniject.files.databinding.ItemFileLinearBinding
import java.io.File
...
//memetype = androidxref.com  در این ادرس انوتع تای هایی که ما نیاز داریم وجود دارد
            var filetype = ""
            txt.text = file.name
    ...
    fun newFolder(newFile: File, position: Int) {
        data.add(0, newFile)
        notifyItemInserted(0)

    }
    fun removeFile(ourFile: File, position: Int) {
        data.remove(ourFile)
